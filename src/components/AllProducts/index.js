...
class AllProductsSection extends Component {
  ...
  onIncrementOffset = ()=>{
    if (this.state.offsetNum <= 20)
    this.setState(prevState => ({ offsetNum: prevState.offsetNum+1}))
    console.log(this.state.offsetNum)
    this.getProducts()
    }

    onDecrementOffset = ()=>{
      if (this.state.offsetNum > 1)
      this.setState(prevState => ({ offsetNum: prevState.offsetNum-1}))
      this.getProducts()
      }   
  ...
  renderProductsList = () => {
    ...
    return (
      ...
      <div className="nextAndpreviewBtn-container">
           <button className = "nextBtn" onClick={this.onDecrementOffset}><BiChevronLeft/></button>
          <p className="current-page">{offsetNum} of 20</p>
          <button className="previewBtn" onClick={this.onIncrementOffset}><BiChevronRight /></button>
          </div>
      </>
    )
  }
